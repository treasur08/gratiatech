...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.is_user_registered(chat_id):
        ...
        elif context.user_data.get('awaiting_task_picture'):
            # Save the picture file_id to the database
            file_id = update.message.photo[-1].file_id
            db.save_task_picture(chat_id, file_id)

            # Mark the task as completed and credit the user
            db.mark_task_as_completed(chat_id)
            db.update_task_balance(chat_id, 100)

            # Send confirmation message
            await context.bot.send_message(chat_id=chat_id, text="Confirming the task you submitted, you will get credited shortly..\nIf you skipped the task your account might be at risk of getting banned.",
                                        reply_markup=ReplyKeyboardMarkup([[KeyboardButton("Back")]], resize_keyboard=True))

            # Clear the state
            context.user_data['awaiting_task_picture'] = False
    ...
    # Unregistered user actions
        ...
        if text in tokens:
            ...
            await context.bot.send_message(chat_id=chat_id, text="Invalid token ❌\n\n Please enter a valid 10 digits GRATIA TECHNOLOGY token.")
